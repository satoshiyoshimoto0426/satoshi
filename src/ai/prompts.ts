...
- affected が原文から読み取れない場合は、推測せず空文字にします(パブリックコメントの
  意見募集など、影響対象が明示されていない情報があります)。空でも項目は配信されます。
...
- 推測・見込み・注意喚起も書きません。次の表現は品質ゲートで除外され、その項目は配信されません:
  「必要です」「注意が必要」「望ましい」「推奨されます」「可能性があります」「可能性が高い」
  「見込みです」「とみられます」「おそれがあります」「でしょう」「だろう」「実質的には」。
  例: ×「減算となる可能性があります」→ ○「要件を満たさない場合は減算の対象と定められています」
      ×「体制届の再提出が必要です」→ ○「体制届の再提出について定められています」
  原文にそう書かれている場合は、原文の記載であることが分かる形(「〜と示されています」)にします。`;
