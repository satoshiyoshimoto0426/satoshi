...
| 秘密情報 | Secret Manager | LINE トークン ×2、Anthropic API キー、通知 Webhook URL |
| AI | Anthropic Claude `claude-opus-5` | 分類と要約(構造化出力) |
| 配信 | LINE Messaging API `POST /v2/bot/message/broadcast` | 全友だち配信 |
| 通知 | Incoming Webhook(Slack / Discord) | 運用通知。URL から自動判別して書式を切り替える |
| IaC | Terraform | 基盤の再現 |
...
| # | チェック | 失敗時 |
|---|---|---|
| Q1 | `sourceUrl` を正規化したうえで、入力アイテムの `canonicalUrl` 集合に含まれる | 項目除外。正規化してから比べるのは、末尾スラッシュや `http://` の表記ゆれだけで正しい項目が消えるのを防ぐため(幻覚 URL は正規化しても集合に無いので緩まない) |
| Q2 | `sourceUrl` に HEAD/GET して 2xx or 3xx(タイムアウト 10 秒、1 回リトライ)。robots.txt が巡回を禁じている URL は「確認省略」として通す ― 存在は Q1/Q3 が担保しており、拒否は「巡回するな」であって「存在しない」ではない。報道サイトは記事ページへのボットを拒否することが多く、除外すると RSS 由来の正当な記事だけのまとめが全滅する | 項目除外(robots 拒否は除外しない) |
...
- 外部アクセス先はソース YAML に列挙されたホストと `api.line.me` `api.anthropic.com` および通知先(`hooks.slack.com` / `discord.com`)のみ(egress をポリシーで制限可能)。
- **内部ネットワーク宛の URL はアプリ側でも遮断する。** 巡回先の候補リンクは外部サイトの
  HTML から取るため、`http://169.254.169.254/`(クラウドのメタデータサーバ)や
  プライベート IP を張られると本文抽出がそこへ到達しうる(SSRF)。
  `HttpClient` がループバック・プライベート・リンクローカル・CGNAT・`.internal` 等を拒否し、
  インフラの egress 制限と二重のガードにする。
